...
courses |>
  group_by(days, time) |>
  summarize(popular_slots = n()) |>
  arrange(desc(popular_slots)) |>
  head(6, courses)
```
...
courses_clean |>
  group_by(dept) |>
  summarize(total_dept_courses = n()) |>
  arrange(desc(total_dept_courses)) |>
  head(6, courses_clean)
...
courses_clean |>
  ...
  arrange(desc(mean_title_length)) |> 
  head(6)
```
